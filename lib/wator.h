...
/**Struttura per rappresentare un punto all'interno del piano*/
typedef struct point {
	int x;
	int y;
} point_t;

/**Struttura che individua un quadrato definendone i due punti estremi della diagonale*/
typedef struct square {
	point_t left;
	point_t right;
}square_t;

/** \brief Definisco un tipo per i puntatori a funzione.

	La dichiarazione del tipo di puntatore a funzione con queste precise caratteristiche è utile in relazione alle funzioni di movimento.
	@see moveDOWN()
	@see moveUP()
	@see moveLEFT()
	@see moveRIGHT()*/
typedef int (*f_pointer_t) (planet_t *, int, int, int *, int *);
...
/**
 * calcola un chronon aggiornando i valori della simulazione riguardanti una secifica sottomatrice
 * 
 * \param pw puntatore al pianeta
 * \param area puntatore alle dimensioni dell'area da aggiornare
 * \param mutexes matrice multidimensionale delle mutex
 * 
 * \return  0 se tutto e' andato bene
 * \return -1 se si e' verificato un errore (setta errno)
 * */
int safely_work(wator_t *pw, square_t *area, pthread_mutex_t **mutexes, int **seen);
/*********************************************************************************************************************/
...
BOOL createLife(planet_t *world, cell_t type, int x, int y, int, int *, int *);
void *error(int nerror);
int findIt(cell_t type, int *buffer, planet_t *pw, int x, int y);

/**
	Macro che alloca n oggetti di tipo type sullo heap
...
/**
	Macro che effettua una free() sul puntatore a una riga di una matrice polimorfa 
	
	\param row
*/
#define FREEROW(row) (free_chunk((void *) row))

/**
	Macro che effettua una free() sul puntatore a una colonna di una matrice polimorfa
	
	\param col
*/
#define FREECOL(col) (free_chunk((void **) col))

/**
	Macro che gestisce l'operazione di modulo aritmetico, prendendo in considerazione anche valori negativi
...
/**
	@name Macro di posizione 
	Set di macro che gestiscono le coordinate relative agli spostamenti nella matrice */
/**@{*/
#define RIGHT(y, max) (MOD((y+1), max))/**< Macro che calcola il valore della y alla destra di quella indicata*/
#define	LEFT(y, max) (MOD((y-1), max))/**< Macro che calcola il valore della y alla sinistra di quella indicata*/
#define UP(x, max) (MOD((x-1), max))/**< Macro che calcola il valore della x sopra quella indicata*/
#define DOWN(x, max) (MOD((x+1), max))/**< Macro che calcola il valore della x sotto quella indicata*/
#define HERE(z, max) (MOD(z, max))/**< Macro che calcola il valore della x o della y valutandone il modulo rispetto alla grandezza massima relativa*/
/**@}*/

/**
	@name Set di colori
	Definisco le costanti necessarie a visualizzare i colori nel terminale
*/
/**@{*/
#define ANSI_COLOR_RED     "\x1b[31m" /**<Costante che indica il colore rosso*/
#define ANSI_COLOR_GREEN   "\x1b[32m" /**<Costante che indica il colore verde*/
#define ANSI_COLOR_BLUE    "\x1b[34m" /**<Costante che indica il colore blu*/
#define ANSI_COLOR_RESET   "\x1b[0m" /**<Costante che reimposta i colori di default*/
/**@}*/
